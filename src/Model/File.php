	...
	class File extends Node{
		...
		protected function _delete($force_delete = false){
			...
			if($force_delete){
				...
				$adapter->changePermissions($dir, 0777);
			}
			try{
				if(!@$adapter->unlink($this->real_path)){
					$e = error_get_last();
					throw new ActionError(sprintf('Could not deleteRecursive file "%s", message: %s',$this->real_path , $e['message']));
				}
			}catch(\Exception $e){
				...
				if($force_delete && isset($dir, $old_dir_permissions)){
					$adapter->changePermissions($dir, $old_dir_permissions);
				}
			}
		}
