	...
	class Directory extends Node implements \ArrayAccess,\IteratorAggregate, \Countable{
		...
		protected function _delete($force_delete = false){
			...
			if($force_delete){
				...
				$adapter->changePermissions($this->real_path, 0777);
			}
			try{
				if(!@$adapter->rmdir($this->real_path)){
					$e = error_get_last();
					throw new Exception\ActionError(sprintf('Could not deleteRecursive newDir "%s", message: %s',$this->real_path , $e['message']));
				}
			}catch(\Exception $e){
				if($force_delete && isset($old_permissions)){
					$adapter->changePermissions($this->real_path, $old_permissions);
				}
				throw $e;
			}
		}
