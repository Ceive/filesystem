...
namespace Jungle\FileSystem\Model {
	...
	use Ceive\Filesystem\Adapter\Adapter;
	...
	abstract class Node{
		...
		public function getPermissions($octal = null){
			if($this->permissions === null){
				$this->permissions = (new Permissions(function($decimal){
					if($this->isActual()){
						if(!@$this->getAdapter()->changePermissions($this->real_path,$decimal)){
							$e = error_get_last();
		...
		public function getAccessTime(){
			if($this->real_path){
				return $this->getAdapter()->getAccessTime($this->real_path);
			}
			return null;
		}
		...
		public function getModifyTime(){
			if($this->real_path){
				return $this->getAdapter()->getModifyTime($this->real_path);
			}
			return null;
		}
		...
		public function getCreateTime(){
			if($this->real_path){
				return $this->getAdapter()->getCreateTime($this->real_path);
			}
			return null;
		}
		...
		protected function _beforeDelete($forceDelete = false, $ignoreError = false,$deletionRoot = null){
			if(!$forceDelete && !$ignoreError && !$this->isDeletable()){
				throw new ActionError('Could not deleteRecursive "'.$this->getAbsolutePath().'", detected contains deleteRecursive permissions access denied');
			}
		}
		...
		protected function _move($newNodePath){
			if(!@$this->getAdapter()->rename($this->real_path, $newNodePath)){
				$e = error_get_last();
				throw new ActionError(sprintf('Could not moveRecursive(rename absolute) node "%s" to "%s", message: %s',$this->getAbsolutePath(), $this->getAbsolutePath($newNodePath)  , $e['message']));
			}
		}
		...
		public function getRelativePath($base, $directory_separator = null){
			...
			if(strpos($path, $parent_path) === 0){
				...
				return FS::normalizePath($relative_path,false,$directory_separator?:$this->getAdapter()->ds());
			}else{
